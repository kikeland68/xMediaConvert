...
-(void) drawRect:(NSRect)dirtyRect
{
	...
	const NSRect k_textRect = NSMakeRect(0.05, 0.65, 0.9, 0.3);
...
#pragma mark Global application instance (assigned in -init)
...
#pragma mark Variables initialitations
-(NSString*) maxCPU
{
...
#pragma mark App Initialitation
- (id)init
{
    assert(!g_appController);
...
- (void) awakeFromNib
{
	...
	// Added by EBJ 2010
	// Draw the lcd panel
	lcdView = [[LcdPanel alloc] initWithFrame:NSMakeRect(0,0,240,48)];
	//LcdPanel *lcdView = [[LcdPanel alloc] initWithFrame:NSMakeRect(0,0,[[mainWindow size ]width / 4],48)];
	// panelLDC is the toolbar button with the LCD View
	//NSString *imagePath = [[NSBundle mainBundle] pathForImageResource:@"monkey.png"];
	[panelLCD setView:lcdView];
	[panelLCD setMinSize:NSMakeSize(240,48)];
	[panelLCD setMaxSize:NSMakeSize(240,48)];
	[panelLCD setLabel:@""];
	[panelLCD setPaletteLabel:@""];
	[panelLCD setToolTip:@""];
	[lcdView clear];
}

#pragma mark Codification
...
#pragma mark Main Enconding Loop
-(void) startNextEncode
{
    ...
    if (!m_isTerminated) {
        ...
        if (!m_someFilesFailed) {
			...
			// Code added by EBJ - Growl notification
/*			GrowlNotifications *gn =[GrowlNotifications instance];
			if (gn) {
				[gn printNotification:self 
						 defaultValue:[NSNumber numberWithInt:1]
						  stickyValue:[NSNumber numberWithInt:1]
							 priority:[NSNumber numberWithInt:4]
								image:nil
								title:@"VideoMonkey"
								 desc:NSLocalizedString(@"ENCODED_SUCCESSFUL", @"All files finished encoding without errors")
						   identifier:@""
						 clickContext:@""];
			}
			NSBeep();
			// end of code added by EBJ */
			
            NSBeginAlertSheet(@"Encoding Successful", nil, nil, nil, [[NSApplication sharedApplication] mainWindow], 
                            nil, nil, nil, nil, 
                            NSLocalizedString(@"ENCODED_SUCCESSFUL", @"All files finished encoding without errors"));
			[lcdView clear];
		}
        else 
		{
...
/*			GrowlNotifications *gn = [GrowlNotifications instance];
						 ...
						 defaultValue:[NSNumber numberWithInt:1]
						  stickyValue:[NSNumber numberWithInt:1]
							 priority:[NSNumber numberWithInt:4]
								image:nil
								title:@"VideoMonkey"
								 desc:NSLocalizedString(@"ENCODED_UNSUCCESSFUL", @"Some files had errors during encoding. See Console for details")
						   identifier:@""
						 clickContext:@""];
			}
			NSBeep();
           ...
           NSBeginAlertSheet(@"Encoding FAILED", nil, nil, nil, [[NSApplication sharedApplication] mainWindow], 
                            nil, nil, nil, nil, 
                            NSLocalizedString(@"ENCODED_UNSUCCESSFUL", @"Some files had errors during encoding. See Console for details"));
			[lcdView clear];
		}
        }
...
#pragma mark Beginning codification
- (IBAction)startEncode:(id)sender
{
	[lcdView setUpperText:NSLocalizedString(@"START_CODIFYING", @"Codifying ...")];
    if (m_runState != RS_PAUSED) {
		...
		//[lcdView setLowerText:[m_fileList objectAtIndex: m_currentEncoding];
...
-(void) setProgressFor: (Transcoder*) transcoder to: (double) progress
{
    if (!transcoder) {
        ...
        [m_progressText setStringValue:NSLocalizedString(@"STARTENCODE", @"Press start to encode")];
        [m_fileNumberText setStringValue:@""];
		//[lcdView setUpperText:NSLocalizedString(@"STARTENCODE", @"Press start to encode")];
		[self updateDockIcon];
        return;
    }
	...
	NSString *timeRemainingStringLcd = nil;
    ...
    if (progress == 0 || timeSoFar < DELAY_FOR_PROGRESS_RESPONSE) {
        timeRemainingString = NSLocalizedString(@"STARTING", @"Starting");
		timeRemainingStringLcd = timeRemainingString;}
    else if (progress == 1) {
        timeRemainingString = NSLocalizedString(@"FINISHING", @"Finishing");
		timeRemainingStringLcd = timeRemainingString;}
    ...
    if (progress == 0 || progress == 1) {
		...
		[lcdView setLowerText:[NSString stringWithFormat:@"%d of %d", m_fileConvertingIndex+1, m_numFilesToConvert]];
		[lcdView setProgress:(float)progress];
        ...
        if (!timeRemainingString) {
            ...
            if (m_lastMinutesRemaining < 0) {
                timeRemainingString = NSLocalizedString(@"ESTIMATINGTIME", @"Estimating time remaining for");
				timeRemainingStringLcd = timeRemainingString; }
            else if (m_lastMinutesRemaining > 0) {
                if (m_lastMinutesRemaining == 1) {
                    timeRemainingString = NSLocalizedString(@"ABOUTAMINUTE", @"About a minute remaining for");
					timeRemainingStringLcd = timeRemainingString; }
                else {
                    timeRemainingString = [NSString stringWithFormat:NSLocalizedString(@"ABOUT_N_MINUTES", @"About %d minutes remaining for"), m_lastMinutesRemaining];
					timeRemainingStringLcd = timeRemainingString; }
            }
            else {
                timeRemainingString = NSLocalizedString(@"LESS_THAN_A_MINUTE", @"Less than a minute remaining for");
				timeRemainingStringLcd = timeRemainingString;}
        }
    }
    
    timeRemainingString = [NSString stringWithFormat:NSLocalizedString(@"FILE", @"%@ file %d..."), timeRemainingString, m_fileConvertingIndex+1];
	timeRemainingStringLcd = timeRemainingString;
    [lcdView setUpperText:timeRemainingStringLcd];
	[m_progressText setStringValue:timeRemainingString];
    [m_fileNumberText setStringValue:[NSString stringWithFormat:NSLocalizedString(@"FILE_OF_TOTAL", @"File %d of %d"), m_fileConvertingIndex+1, m_numFilesToConvert]];
    [m_fileListController reloadData];
}
...
- (BOOL) validateToolbarItem:(NSToolbarItem *)theItem
{
    if (theItem == m_startEncodeItem) {
        [m_startEncodeItem setLabel:(m_runState == RS_PAUSED) ? @"Resume" : NSLocalizedString(@"START", @"Start")];
...
- (void)pathControl:(NSPathControl *)pathControl willPopUpMenu:(NSMenu *)menu
{
    ...
    [sameFolderItem setTitle: NSLocalizedString(@"SAVETO", @"Same folder as input file")];
